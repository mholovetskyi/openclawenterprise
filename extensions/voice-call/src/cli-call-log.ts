...
export function registerVoiceCallLogs(params: {
  ...
  params.root
    ...
    .action(async (options: { file: string; since?: string; poll?: string }) => {
      ...
      // Read directly and treat ENOENT as "no JSONL log" instead of an
      // existsSync pre-check, so a file removed between check and read cannot
      // crash the command (TOCTOU hardening).
      let initial: Buffer | null = null;
      if (path.basename(file) !== "calls.jsonl") {
        try {
      ...
      if (initial !== null) {
        ...
        for (;;) {
          try {
            // Open first and fstat the same descriptor so the size we compare
            // against always describes the file we read from (no stat/open
            // TOCTOU race when the log is rotated or replaced mid-poll).
