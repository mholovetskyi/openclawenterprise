...
# Enterprise hardening: strip test/spec fixtures and secret-scanning metadata
# before runtime assets are copied into the final image. Repository test files
# are already excluded via .dockerignore, but published node_modules packages
# can still ship test fixtures containing realistic-looking fake credentials
# (RSA keys, JWTs, API tokens) that trigger false-positive secret-scanner
# (Trivy/Gitleaks) alerts on the image.
