...
export class InMemoryRBACStore implements RBACStore {
  ...
  async listRoles(): Promise<Role[]> { return [...this.roles.values()]; }
  async getRole(id: string): Promise<Role | null> { return this.roles.get(id) ?? null; }
  async upsertRole(role: Role): Promise<void> { this.roles.set(role.id, role); }
  ...
  async getUserByChannelId(channel: string, channelUserId: string): Promise<User | null> {
    return (
      [...this.users.values()].find((u) => u.channelIds?.[channel] === channelUserId) ?? null
    );
  }
  async upsertUser(user: User): Promise<void> { this.users.set(user.id, user); }
  ...
  async getGroup(id: string): Promise<Group | null> { return this.groups.get(id) ?? null; }
  async upsertGroup(group: Group): Promise<void> { this.groups.set(group.id, group); }
  async deleteGroup(id: string): Promise<void> { this.groups.delete(id); }
  ...
  async deleteAgentIdentity(id: string): Promise<void> { this.agents.delete(id); }
}
