...
export function normalizeChatSendRequest(params: {
  ...
  if (enterpriseSanitized.injectionDetected) {
    void auditLog({
      ...
      category: "security",
      actor: { type: "user", id: p.sessionKey ?? "unknown" },
    ...
    return { ok: false, error: "Message rejected: potential prompt injection detected" };
  }
  ...
  const inboundMessage = enterpriseSanitized.sanitized;
