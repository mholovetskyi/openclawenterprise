...
export async function startGatewayServerCore(
  ...
  try {
    ...
    if (!gatewayKernel.minimalTestGateway) {
      try {
        enterpriseHandle = await initEnterprise(gatewayKernel.cfgAtStart);
      } catch (enterpriseInitError) {
        log.warn(`enterprise: init failed (non-fatal): ${formatErrorMessage(enterpriseInitError)}`);
      }
    }
  } catch (err) {
          ...
          },
          {
            ...
            run: async () => {
              ...
              try {
                ...
                log.warn(`enterprise: shutdown error (ignored): ${formatErrorMessage(error)}`);
              }
            },
          },
