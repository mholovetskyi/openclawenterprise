...
function requireIAM(): { store: RBACStore; rbac: RBACEngine; tokens: TokenStore | null } {
  ...
  if (!h) {
    throw new Error("Enterprise IAM not initialized");
  }
...
function requireAudit(): AuditStorage {
  ...
  if (!s) {
    throw new Error("Enterprise audit not initialized");
  }
  return s;
}
...
export function registerEnterpriseMethods(server: GatewayServer): void {
  ...
  server.registerMethod(
    ...
    async (params: { tenantId?: string; limit?: number; offset?: number }) => {
  ...
  server.registerMethod(
    ...
    async (params: { jti?: string; subjectId?: string }) => {
      const { tokens } = requireIAM();
      if (!tokens) {
        return { revoked: 0 };
      }
  ...
  server.registerMethod("enterprise.gdpr.erase", async (params: { userId: string }) => {
    ...
    const audit = getAuditStorage();
    const result = await gdprEraseUser(
      params.userId,
      store,
      tokens ?? undefined,
      audit ?? undefined,
  ...
  server.registerMethod(
    ...
    async (params: { userId: string; code: string }) => {
      const { store } = requireIAM();
      const user = await store.getUser(params.userId);
      if (!user) {
        throw Object.assign(new Error("User not found"), { code: "NOT_FOUND" });
  ...
  server.registerMethod(
    ...
    async (params: { userId: string; secret: string; code: string }) => {
      const { store } = requireIAM();
      const user = await store.getUser(params.userId);
      if (!user) {
        throw Object.assign(new Error("User not found"), { code: "NOT_FOUND" });
  ...
  server.registerMethod(
    ...
    async (params: { userId: string; ipAddress: string }) => {
      const { store } = requireIAM();
      const user = await store.getUser(params.userId);
      if (!user) {
