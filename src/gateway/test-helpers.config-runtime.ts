...
export function createGatewayConfigModuleMock(actual: GatewayConfigModule): GatewayConfigModule {
  ...
  const readConfigFileSnapshot = async (): Promise<ConfigFileSnapshot> => {
    ...
    try {
      ...
      // Single read instead of access-then-read: missing files are detected via
      // ENOENT from the read itself so the existence check cannot race.
  ...
  const loadTestConfig = () => {
    ...
    try {
      ...
      // Missing file or parse failure — fall back to empty config.
      fileConfig = {};
    }
