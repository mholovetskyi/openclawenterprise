...
export async function logGatewayStartup(params: {
  ...
  // Enterprise: emit a security warning when the gateway is exposed on non-loopback interfaces.
  if (isExposedBindHost(params.bindHost)) {
    ...
    params.log.warn(lines.join("\n"));
  }

  // Enterprise: warn when auth mode is "none" — only acceptable on loopback.
  if (params.cfg.gateway?.auth?.mode === "none") {
    params.log.warn(
      'gateway auth mode is "none" — all connections are accepted without credentials',
    );
  }
