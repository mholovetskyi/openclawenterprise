// Verifies implicit provider secret wiring for NVIDIA, NemoClaw, MiniMax portal, and vLLM.
import { describe, expect, it, vi } from "vitest";
import type { ModelDefinitionConfig, ModelProviderConfig } from "../config/types.models.js";
import { resolveEnvApiKey } from "./model-auth-env.js";
import {
  buildNemoClawProvider,
  resolveNemoClawImplicitProvider,
} from "./models-config.providers.nemoclaw.js";
import {
...
describe("NemoClaw provider", () => {
  ...
  it("should include nemoclaw when NVIDIA_API_KEY is configured (fallback)", () => {
    const provider = resolveNemoClawImplicitProvider({
      env: { NVIDIA_API_KEY: "test-key" } as NodeJS.ProcessEnv,
    });
  ...
  it("should include nemoclaw when NEMOCLAW_API_KEY is configured", () => {
    const provider = resolveNemoClawImplicitProvider({
      env: { NEMOCLAW_API_KEY: "nemoclaw-test-key" } as NodeJS.ProcessEnv,
    });
    expect(provider).toBeDefined();
    expect(provider?.apiKey).toBe("NEMOCLAW_API_KEY");
  });

  it("should not resolve nemoclaw without credentials", () => {
    expect(resolveNemoClawImplicitProvider({ env: {} as NodeJS.ProcessEnv })).toBeUndefined();
  });
});
