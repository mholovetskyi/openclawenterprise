...
import { resolveNemoClawImplicitProvider } from "./models-config.providers.nemoclaw.js";
import type {
...
export async function resolveImplicitProviders(
  ...
  // Enterprise hook: register the NemoClaw provider (NVIDIA-hosted Nemotron
  // catalog behind NEMOCLAW_API_KEY, falling back to the NVIDIA credential).
  const requestedProviderIds = params.providerDiscoveryProviderIds?.map(normalizeProviderId);
  if (
    !providers.nemoclaw &&
    (requestedProviderIds === undefined || requestedProviderIds.includes("nemoclaw"))
  ) {
    const nemoClawProvider = resolveNemoClawImplicitProvider({ env, getAuthStore });
    if (nemoClawProvider) {
      providers.nemoclaw = nemoClawProvider;
    }
  }
